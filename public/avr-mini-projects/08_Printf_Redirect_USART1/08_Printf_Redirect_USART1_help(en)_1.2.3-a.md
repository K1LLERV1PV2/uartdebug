...
Redirect `printf()` output to UART1 for blocking formatted-text transmission.

## Full Mini-Project Description

This mini-project initializes UART1 and redirects the standard C `stdout` stream to it.

After this redirection, text produced by `printf()` is transmitted through UART1. The example sends an incrementing counter every 500 ms:
...
The project uses blocking UART transmission. Each character produced by `printf()` is passed to `USART_PrintChar()`, which waits until the USART transmit buffer can accept another byte and then writes the character to `USART1.TXDATAL`.
...
Project 07 redirects `printf()` through UART0. Project 08 uses the same blocking `printf()` redirection method through UART1.
...
- a second adapter for receiving and displaying the UART1 text generated by `printf()`.
...
Connect the second USB-UART adapter to the microcontroller for observing the UART1 text output.
...
- character transmission through USART1;
- formatted text output;
- hardware connection to the receiving USB-UART adapter.

## What This Mini-Project Is For

This mini-project provides a ready-to-use UART1 version of the `printf()` debugging technique introduced in Project 07.

A separate UART1 version is useful when UART0 is already occupied, when two serial channels are required, or when debug output should be separated from another communication channel.

After `stdout` is redirected to UART1, `printf()` can display:
...
- Connecting the receiving USB-UART adapter to `PB2` instead of `PA1`.
- Forgetting that this project uses UART1, not UART0.
...
This polling loop waits until the USART1 transmit buffer can accept another byte.
...
This writes one character produced by `printf()` to the USART1 transmit data register.
...
`USART_Init()` configures UART1 and redirects `stdout`.
...
1. Replace `USART1.` with `USART1.`.
2. Replace `PORTA.DIRSET = PIN1_bm;` with `PORTA.DIRSET = PIN1_bm;`.
...
UART1 is configured and `stdout` is redirected before the main loop starts.
...
This mini-project is intended for tinyAVR 2-series devices that provide USART1 on `PA1` and have compatible USART registers, together with an XC8 standard I/O implementation supporting `FDEV_SETUP_STREAM`.
