...
test("canvas installation includes its complete runtime and retains protected data paths", (t) => {
  ...
  for (const filename of ["avr-ai-runtime.js", "avr-canvas-contract.js", "avr-documentation-lookup.js", "avr-knowledge.js", "avr-documentation-markers.js"]) {
    assert.ok(installer.includes(filename), `The installer must copy ${filename}`);
  }
  ...
  assert.match(installer, /install -d -o uartai -g uartai -m 0700 "\$\{documentation_root\}"/);
  assert.match(unit, /AI_DOCUMENTATION_CACHE_DIR=\/var\/lib\/uartdebug-ai\/documentation/);
  assert.match(unit, /ReadWritePaths=\/var\/lib\/uartdebug-ai\/documentation \/var\/lib\/uartdebug-ai\/data/);
  assert.doesNotMatch(installer, /loadAiSkillCatalog|install-ai-rule-pack|ai\/skills/);
  assert.doesNotMatch(unit, /AI_RULE_PACK_ROOT|AI_DRAFTS_DIR|AI_MAX_DRAFTS/);
  const bash = process.platform === "win32" ? "C:\\Program Files\\Git\\bin\\bash.exe" : "bash";
  if (process.platform === "win32" && !fs.existsSync(bash)) return t.skip("Git Bash is not installed");
  for (const script of ["install-ai-service.sh", "smoke-ai-service.sh"]) {
    execFileSync(bash, ["-n", path.join(repositoryRoot, "backend/deploy", script)], { encoding: "utf8" });
  }
});
...
test("canvas smoke check verifies retired routes and avoids paid generation", async (t) => {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.url === "/api/avr/ai/status") return res.end(JSON.stringify({ ok: true, contract: "uartdebug-canvas/v1", accessRequired: false, configured, rules: { packageId: "canvas-v2" }, knowledge: { devices: [{ mcu: "ATtiny1624" }] } }));
    if (req.url === "/api/avr/ai/auth/session") return res.end(JSON.stringify({ ok: true, mode: "public" }));
    if (req.url === "/api/avr/ai/canvas") {
      res.statusCode = 503;
      return res.end(JSON.stringify({ ok: false, code: "api_key_not_configured" }));
    }
    res.statusCode = 404;
  ...
  const args = [path.join(repositoryRoot, "backend/deploy/smoke-ai-service.sh"), `http://127.0.0.1:${server.address().port}`];
  const run = () => promisify(execFile)(bash, args, { encoding: "utf8", timeout: 20000 });
  ...
  for (const route of ["POST /api/avr/ai/respond", "POST /api/avr/ai/generate", "GET /api/avr/ai/skills"]) assert.ok(requests.includes(route));
  configured = false;
  const withoutKey = await run();
  assert.match(withoutKey.stdout, /canvas-without-key=503/);
  assert.equal(requests.filter((value) => value === "POST /api/avr/ai/canvas").length, 1);
});

test("the rollback guard blocks a canvas database from a pre-canvas backend", (t) => {
  const bash = process.platform === "win32" ? "C:\\Program Files\\Git\\bin\\bash.exe" : "bash";
  if (process.platform === "win32" && !fs.existsSync(bash)) return t.skip("Git Bash is not installed");
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "uartdebug-rollback-"));
  t.after(() => fs.rmSync(temporaryDirectory, { recursive: true, force: true }));
  const release = "20260924-120000-abcdef0";
  const roots = ["public", "backend", "python"].map((name) => path.join(temporaryDirectory, name));
  for (const root of roots) fs.mkdirSync(path.join(root, release), { recursive: true });
  ...
  fs.writeFileSync(database, "Test-only placeholder read through the controlled sqlite3 stub.\n");
  const bin = path.join(temporaryDirectory, "bin");
  fs.mkdirSync(bin);
  fs.writeFileSync(path.join(bin, "sqlite3"), "#!/usr/bin/env bash\nprintf '3\\n'\n", { mode: 0o755 });
  const posix = (value) => process.platform === "win32" ? value.replace(/\\/g, "/").replace(/^([A-Za-z]):/, (_, drive) => `/${drive.toLowerCase()}`) : value;
  const script = path.join(repositoryRoot, "backend/deploy/guard-ai-access-schema-rollback.sh");
  const args = ["-c", 'export PATH="$1:$PATH"; shift; exec bash "$@"', "guard-test", posix(bin), posix(script), ...roots.map(posix), release, posix(database)];
  assert.throws(() => execFileSync(bash, args, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }), (error) => {
    assert.equal(error.status, 78);
    assert.match(error.stderr, /live AI database schema 3 is newer than target AI_ACCESS_SCHEMA_VERSION 2/);
  ...
  const output = execFileSync(bash, args, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
...
test('release verification detects mismatched running knowledge or rules', async t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'canvas-release-check-'));
  ...
  const script = path.join(repositoryRoot, 'backend/deploy/verify-ai-release.js');
  fs.writeFileSync(file,JSON.stringify(status));
  execFileSync(process.execPath,[script,file]);
  for (const field of ['rules','knowledge']) {
    const mismatch=structuredClone(status); mismatch[field].digest='0'.repeat(64);
    fs.writeFileSync(file,JSON.stringify(mismatch));
    assert.throws(()=>execFileSync(process.execPath,[script,file],{stdio:'pipe'}),error=>error.status===1);
  }
});
