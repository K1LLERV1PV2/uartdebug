...
const directory = path.join(root, "backend/ai/knowledge/attiny162x/1.1.0");
...
const manifest = {
  ...
  sources: originals,
  reference: { corpus: "reference/corpus.json", registers: "reference/dfp-registers.json", reviewedFacts: "reference/reviewed-facts.json", pageCount: corpus.documents.reduce((total, doc) => total + doc.pageCount, 0), sectionCount: corpus.documents.reduce((total, doc) => total + doc.sections.length, 0), reviewedFactCount: reviewed.length },
  recipes: definitions.map(([id, folders]) => ({ id, description: fs.readFileSync(path.join(directory, `recipes/${id}/SKILL.md`), "utf8").match(/^description: (.+)$/m)[1].trim(), file: `recipes/${id}/SKILL.md`, sourceIds: originals.map((source) => source.id), reviewedFactIds: reviewed.filter((fact) => fact.recipeIds?.includes(id)).map((fact) => fact.id), lineage: folders, verification: "reviewed-recipe-with-bounded-composition-compile-evidence", compilerEvidence: "compiler-evidence.json" })),
  lineage,
  files,
};
